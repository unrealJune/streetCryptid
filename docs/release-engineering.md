...
> How a commit on `main` becomes a TestFlight build and a Play internal-track release,
> and how pull-request builds are made. Split out of the [README](../README.md).
...
`scripts/upload-build.sh`, to a self-hosted app-distribution server reached over a Tailscale
ephemeral node, and a single notify job posts both install links to a per-PR Discord thread
(`scripts/notify-discord-thread.sh`). No cloud-build quota is consumed and no binary is uploaded
to Expo. The distribution host, its base URL, and each install URL are registered as log masks and
written only to `$GITHUB_OUTPUT`, so a public fork's logs never carry them.
...
3. Add the distribution secrets the upload and notify jobs need: `DISTRIBUTOR_BASE_URL` and
   `DISTRIBUTOR_TOKEN` for the app-distribution server, `TS_OAUTH_CLIENT_ID` / `TS_OAUTH_SECRET`
   and `TS_TAILNET` for the ephemeral Tailscale node that reaches it,
   and `DISCORD_WEBHOOK_URL` for the thread the install links are posted to.
4. Approve each pending workflow run separately, including every run created after new commits.
