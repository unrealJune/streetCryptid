...
describe('pair-link codec', () => {
  ...
  it('still accepts legacy scpair1: tokens and links', () => {
    const legacy = 'scpair1:deadbeefcafe';
    expect(isPairToken(legacy)).toBe(true);
    expect(isPairLink(legacy)).toBe(true);
    expect(decodePairLink(legacy)).toBe(legacy);
    expect(decodePairLink(encodePairLink(legacy))).toBe(legacy);
  });
  ...
  it('recognizes pair links and tokens', () => {
    ...
    expect(isPairToken('scpair1:')).toBe(false);
