 ...
 * sealed envelope gets from your phone to a friend's: direct fails and retries, mutual relay
 * survives your friend going dark, the stash holds a copy until she comes back and then drops
 * it. The point of animating any of this is that the failure is the interesting part, and a
 * still frame cannot show a failure — only its aftermath.
 *
...
/**
 * Two phones facing each other. One envelope leaves, dissolves two thirds of the way across
 * because there is no route, and a retry lands.
 */
function directScene(t: number, size: StageSize): DeliveryScene {
  const pw = 112;
  const ph = Math.min(252, size.height - 60);
  const y = (size.height - ph) / 2 - 12;
  const a: StageRect = { x: 12, y, w: pw, h: ph };
  const b: StageRect = { x: size.width - 12 - pw, y, w: pw, h: ph };
  const ca = centerOf(a);
  const cb = centerOf(b);
  const arrived = t >= 0.74;
  const failing = t > 0.3 && t < 0.5;

  const payloads: PayloadNode[] = [];
  // First attempt — it never gets there.
  if (t < 0.4) {
    const p = ease(seg(t, 0.12, 0.3));
    payloads.push({
      key: 'first',
      x: ca.x + (cb.x - ca.x) * p * 0.62,
      y: ca.y - Math.sin(p * Math.PI) * 16,
      scale: 1.35,
      alpha: seg(t, 0, 0.12),
      resolve: 0,
      scatter: seg(t, 0.3, 0.4),
      seed: 1,
    });
  }
  // Retry.
  if (t >= 0.5 && t < 0.8) {
    const p = ease(seg(t, 0.5, 0.74));
    payloads.push({
      key: 'retry',
      x: ca.x + (cb.x - ca.x) * p,
      y: ca.y - Math.sin(p * Math.PI) * 16,
      scale: 1.35,
      alpha: 1 - seg(t, 0.72, 0.78),
      resolve: 0,
      scatter: 0,
      seed: 4,
    });
    ...
    phones: [
      {
        ...
        wake: 1,
        dim: false,
        lit: (1 - Math.abs(0.5 - seg(t, 0.4, 0.5)) * 2) * 0.6,
      },
      {
        id: 'friend',
        rect: b,
    ...
    routes: [
      {
        ...
        from: ca,
        to: cb,
        strength: t > 0.48 ? seg(t, 0.48, 0.74) : 0.15 * (1 - seg(t, 0.3, 0.4)),
      },
    ],
    payloads,
    slab: null,
...
export function buildDeliveryScene(mode: DeliveryMode, t: number, size: StageSize): DeliveryScene {
  ...
  if (mode === 'mutual') return mutualScene(wrapped, size);
  if (mode === 'stash') return stashScene(wrapped, size);
  return directScene(wrapped, size);
}
