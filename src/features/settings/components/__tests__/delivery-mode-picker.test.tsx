...
const ALL: DeliveryAvailability = { stashConfigured: true };
...
describe('DeliveryModePicker', () => {
  ...
  function render(availability: DeliveryAvailability, selected: 'mutual' | 'stash') {
  ...
  it('offers both routes and reports the selected one', () => {
  ...
  it('reports the choice by id, not by index', () => {
    const { onSelect, segment } = render(ALL, 'mutual');
  ...
  it('still shows a route this build cannot offer, but refuses to select it', () => {
    ...
    const { onSelect, segment } = render(NO_STASH, 'mutual');
