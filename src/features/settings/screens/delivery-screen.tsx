...
const UNAVAILABLE_COPY = {
  ...
  'relay-unsupported': '',
  'mutual-unsupported':
    'This phone is running a build whose native core cannot carry for a mutual yet. Your trail still travels direct.',
} as const;

/**
 * Delivery options — how your location travels.
 *
 * The three routes are genuine alternatives rather than switches, so they are picked as one
 ...
 * `delivery-timeline.ts` for what each loop is actually claiming.
 *
...
export default function DeliveryScreen() {
  ...
  const availability = useMemo<DeliveryAvailability>(
    () => ({
      ...
      mutualSupported: snapshot?.delivery.mutualSupported ?? false,
    }),
    [snapshot?.delivery.stashConfigured, snapshot?.delivery.mutualSupported]
  );
  const storedMode = snapshot?.delivery.mode ?? 'direct';
      ...
      <View style={styles.hero}>
        ...
        <View style={styles.copy}>
          ...
          {selectedOption && !selectedOption.available ? (
            <ThemedText type="small" style={{ color: chrome.amber }}>
              {UNAVAILABLE_COPY[selectedOption.unavailableReason ?? 'mutual-unsupported']}
            </ThemedText>
          ) : null}
        </View>
        ...
        {downgraded ? (
          <ThemedText type="small" style={{ color: chrome.amber }}>
            ...
            travelling direct. Your choice is remembered.
          </ThemedText>
        ) : null}
      </View>
