...
# Each of these three has actually broken a build job in this pipeline, and none of them can be
# caught by running the code on a developer machine: the first two are macOS-only, and the third
# needs POSIX pipe semantics that Git Bash on Windows does not reproduce. So they are checked as
# source rules, which works everywhere.
lint() {
  local script="$1" pattern="$2" reason="$3"
  # Comments are stripped first, or the comment explaining a rule trips that rule.
  if grep -v '^[[:space:]]*#' "$repo_root/scripts/$script" | grep -nE "$pattern"; then
    fail "$script: $reason"
  fi
}

for script in build-profile.sh build-report.sh; do
  lint "$script" '(declare|local)[[:space:]]+-A|mapfile|readarray|\$\{[A-Za-z_]+(,,|\^\^)\}' \
    "bash 4 construct, and the macOS runner's /bin/bash is 3.2"
  lint "$script" 'head -n' \
    "truncates with head; head exits early and SIGPIPEs the writer feeding it, which set -o pipefail turns into a failed report. Use awk 'NR <= n', which drains its input"
  lint "$script" 'sed.*\\t' \
    "sed with a \\t in the script; BSD sed writes a literal t, and the iOS job runs BSD sed"
  # `wc -l` is fine, but BSD pads its output with leading spaces, so it always needs stripping.
  if grep -v '^[[:space:]]*#' "$repo_root/scripts/$script" |
    grep -nE 'wc -l' | grep -qv 'tr -d'; then
    fail "$script: wc -l without stripping whitespace; BSD wc pads its output and GNU wc does not"
  fi
done
...
# ENOUGH ROWS TO OVERFLOW A PIPE, which is a different requirement from "more rows than the report
# shows" and the reason this bug reached CI.
#
# The renderer truncates each table. Doing that with `head` makes head exit early, and the `sort`
# feeding it dies of SIGPIPE, which `set -o pipefail` turns into a failed report. But SIGPIPE only
# happens if the writer is still writing when the reader leaves: a few dozen short lines are a
# single write that completes first, so a fixture that merely exceeded the display cap passed
# happily while the real ~400-crate graph failed. These are sized in the tens of thousands of bytes
# so the writer is guaranteed to still be going. Generated by awk rather than a shell loop because
# a few thousand `printf` forks is not free.
awk 'BEGIN { for (i = 1; i <= 5000; i++) printf "filler-crate-%05d\t%d\n", i, i * 7 }' \
  >> "$crates_file"

# Likewise for meta.tsv: meta_value used to return on its first match, abandoning the process
# substitution mid-write, and whether that raised "printf: write error: Broken pipe" depended on
# the remainder fitting in the pipe buffer.
awk 'BEGIN { for (i = 1; i <= 5000; i++) printf "filler_note_%05d\t%d\n", i, i }' >> "$meta_file"
...
awk 'BEGIN { for (i = 1; i <= 5000; i++) printf ":app:fillerTask%05d\tEXECUTED\t%d\n", i, i }' \
  >> "$SC_BUILD_PROFILE_DIR/gradle-tasks.tsv"
...
EOF
# Deliberately fast, so the real phases stay at the top of a table sorted by duration: the point of
# the fillers is pipe volume, not displacing the rows the assertions below look for.
awk 'BEGIN { for (i = 1; i <= 5000; i++) printf "FillerPhase%05d (2 tasks) | 0.%03d seconds\n", i, i % 1000 }' \
  >> "$test_root/xcode-buildlog/streetCryptid-streetCryptid.log"

report="$test_root/report.md"
report_err="$test_root/report.err"
SC_BUILD_SECONDS=1277 \
  SC_CACHE_JS_HIT=true \
  SC_CACHE_CARGO_HIT=false \
  SC_CACHE_NATIVE_HIT="$sentinel" \
  SC_CACHE_GRADLE_HIT=true \
  bash "$repo_root/scripts/build-report.sh" android > "$report" 2> "$report_err" ||
  fail "the renderer exited non-zero: $(cat "$report_err")"

# Silence on stderr is an assertion in its own right. The renderer runs under `set -o pipefail`,
# where a truncating `head` or an abandoned process substitution shows up first as broken-pipe
# noise and then as a failed step -- which is exactly how this reached CI once.
if [[ -s "$report_err" ]]; then
  fail "the renderer wrote to stderr: $(cat "$report_err")"
fi
...
SC_BUILD_SECONDS=985 \
  ...
  bash "$repo_root/scripts/build-report.sh" ios > "$ios_report" 2> "$report_err" ||
  fail "the iOS renderer exited non-zero: $(cat "$report_err")"

if [[ -s "$report_err" ]]; then
  fail "the iOS renderer wrote to stderr: $(cat "$report_err")"
fi
